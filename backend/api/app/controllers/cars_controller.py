...
from app.database import collection_cars, database
from app.schemas.cars_schema import CarCreateSchema, serialize_car, serialize_cars

def get_cars() -> list:
    return serialize_cars(collection_cars.find())

def create_car(car: CarCreateSchema) -> dict:
    maker = car.maker
    car_data = car.car.dict()

    existing_maker = collection_cars.find_one({"maker.name": maker.name})
    if existing_maker:
        existing_maker["maker"]["cars"].append(car_data)
        collection_cars.update_one({"_id": existing_maker["_id"]}, {"$set": existing_maker})
    else:
        new_maker = {"maker": {"name": maker.name, "cars": [car_data]}}
        collection_cars.insert_one(new_maker)
    return {"message": "Car created successfully"}

def get_car_by_model(model: str) -> list:
    return serialize_cars(collection_cars.find({"maker.cars.model": model}))

def get_cars_by_maker(maker_name: str) -> list:
    maker_data = collection_cars.find_one({"maker.name": maker_name})
    if not maker_data:
        raise HTTPException(
